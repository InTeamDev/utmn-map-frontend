...
const ImageCard: React.FC<ImageCardProps> = ({ src, alt }) => {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (e.touches.length === 2 && initialDistance !== null) {
      ...
      const relativeX = (centerX - wrapperRect.left - positionRef.current.x) / scaleRef.current
      const relativeY = (centerY - wrapperRect.top - positionRef.current.y) / scaleRef.current
      ...
      frameRef.current = requestAnimationFrame(() => {
        if (imageRef.current) {
          imageRef.current.style.transform =
            `translate(${newPosition.x}px, ` + `${newPosition.y}px) scale(${scaleRef.current})`
        }
        setPosition(newPosition)
        positionRef.current = newPosition
      })
